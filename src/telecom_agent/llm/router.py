...
1. **Pin the provider per session, not per call.** Once a session is pinned, every node
   ...
   same model. Failover flips the pin *once* and stays flipped (``degraded=True``).
...
4. **Degraded mode is visible** — the router exposes ``is_degraded`` and increments a
   failover counter that observability turns into ``llm_failover_total``.
...
# Optional import of failover counter hook; observability sets this if present.
_FAILOVER_HOOK = None


def set_failover_hook(fn) -> None:  # pragma: no cover - wiring
...
class LLMRouter:
    def __init__(self, settings: Settings) -> None:
        ...
        # Backup is Groq only when the primary is Foundry (README §4). Fake/Groq primary
        # have no chat backup; both-down then falls to the rule-based path in classify.
        self._backup_kind: Provider | None = (
            Provider.GROQ if self._primary_kind == Provider.AZURE_FOUNDRY else None
        )
        ...
        if self._backup_kind:
            self._breakers[self._backup_kind] = CircuitBreaker(
    ...
    def ensure_session(self, session_id: str) -> Provider:
        ...
        if session_id not in self._sessions:
            # If the primary breaker is already open, start the session degraded.
            start = self._primary_kind
            degraded = False
            if self._breaker_open(self._primary_kind) and self._backup_kind:
                start, degraded = self._backup_kind, True
    ...
    def chat(
        ...
        *,
        ...
        """Run a chat completion on the session's pinned provider, failing over once to
        the backup if the primary errors and failover is enabled."""
        self.ensure_session(session_id)
        sess = self._sessions[session_id]

        order = [sess.provider]
        if self._s.llm_failover_enabled and self._backup_kind and self._backup_kind not in order:
            order.append(self._backup_kind)

        last_exc: ProviderError | None = None
        for kind in order:
            if self._breaker_open(kind):
                last_exc = BreakerOpen(f"{kind} breaker open")
                continue
            try:
                result = self._attempt(kind, messages, alias, temperature, max_tokens, json_mode)
            except ContentFiltered:
                # Do not retry or fail over on a content-filter block — surface it.
                raise
            ...
            if kind != sess.provider:  # we failed over → flip the pin, mark degraded
                sess.provider = kind
                sess.degraded = True
                if _FAILOVER_HOOK:
                    _FAILOVER_HOOK(reason=type(last_exc).__name__ if last_exc else "unknown")
            if sess.budget:
    ...
    def health(self) -> dict[str, bool]:
        ...
        out: dict[str, bool] = {}
        for kind in filter(None, [self._primary_kind, self._backup_kind]):
            try:
